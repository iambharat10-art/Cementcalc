...
async def admin_panel(request: Request):
    ...
    admin_rows = "\n".join([f'<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:rgba(255,255,255,0.03);border-radius:8px;margin-bottom:6px"><span style="flex:1;font-family:monospace;font-size:13px">{e}</span>{"<span style=\\"color:#22c55e;font-size:10px;font-weight:700\\">SUPER</span>" if e.lower().strip() == SUPER_ADMIN.lower() else f"<button onclick=\\"removeAdmin(\\'{e}\\')\\" style=\\"background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);color:#ef4444;padding:3px 10px;border-radius:5px;font-size:10px;cursor:pointer\\">Remove</button>"}</div>' for e in admin_list])
