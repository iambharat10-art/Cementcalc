...
DASHBOARDS = [
    {
        ...
        "description": "Belt conveyor mechanical design per CEMA and IS 11592 — motor sizing, idler selection, belt tension analysis, and auxiliary equipment sizing.",
    ...
    },
    {
        ...
        "description": "Equipment sizing across the full cement plant flowsheet — crushers, raw mills, kilns, coolers, cement mills, and auxiliary systems.",
    ...
    },
    {
        ...
        "description": "Bond Work Index method for grindability assessment and grinding power estimation for ball mills and VRMs.",
    ...
    },
    {
        ...
        "description": "Rotary kiln thermal balance with input/output heat streams, shell radiation losses, and Specific Heat Consumption (SHC) calculation.",
    ...
    },
    {
        ...
        "description": "Pipe sizing for water, compressed air, and slurry systems — Darcy-Weisbach pressure drop, velocity verification, and standard pipe schedule selection.",
    ...
    },
    {
        ...
        "subtitle": "Engineering Project Tracker",
        "description": "Engineering project tracker for cement plant construction — drawing register, submittal log, RFI tracker, and milestone management.",
    ...
    },
    {
        ...
        "subtitle": "Cement Chemistry · Modulus Targets",
        "description": "Raw mix design using LSF, SM, and AM target moduli with Bogue compound calculation and component optimization.",
    ...
    },
    {
        ...
        "description": "Capacity sizing for silos, hoppers, bunkers, and stockpiles with geometry optimization and mass flow analysis.",
    ...
    },
    {
        ...
        "description": "Stockpile volume and footprint sizing for conical, longitudinal, and trapezoidal pile geometries — limestone and bulk material storage.",
    ...
    },
    {
        ...
        "description": "Cement and raw meal silo sizing — inverted cone and flat-bottom configurations with reverse calculation for optimal diameter and height.",
    ...
    },
    {
        ...
        "title": "Bag Filter & Dedusting Calculator",
        "subtitle": "Process &amp; Nuisance Filter Sizing",
        "description": "Bag filter sizing for up to 6 venting points with pipe sizing, fan selection, Nm³/m³ converter, and standard motor selection for cement plant dedusting.",
        "file": "Bag_Filter_Dedusting_Calculator.html",
        "icon": "🌬️",
        "category": "Utilities",
        "restricted": False,
        "color": "#06b6d4",
        "keywords": "bag filter calculator, dedusting, nuisance filter, bag filter sizing, fan selection, pipe sizing, cement plant dust collection",
    },
    {
        "id": "compressor-power",
        "title": "Compressor Power & Temperature",
        "subtitle": "Thermodynamic Sizing",
        "description": "Compressor shaft power and discharge temperature for isentropic and polytropic compression — scenario comparison with complete thermodynamic step-through.",
    ...
    {
        "id": "vrm-heat-balance",
        "title": "VRM Thermal & Material Balance",
        "subtitle": "Vertical Roller Mill Engineering Workbench",
        "description": "Coupled thermal and material balance for Vertical Roller Mills — raw mill, coal mill, and cement mill applications. Includes fan sizing, dust mass balance, and operating scenario library.",
        "file": "VRM_Heat_Mass_Balance.html",
        "icon": "🌀",
        "category": "Grinding",
        "restricted": True,
        "color": "#10b981",
     ...
     {        
        ...
        "file": "cement-tech-reference.html",
        "icon": "🏭",
        "category": "Plant Design",
        "restricted": False,
        "color": "#ef4444",
        "keywords": "cement machinery, cement plant equipment, machinery selection, process equipment, engineering reference, cement technology",
    },
]
...
<div class="popup-hdr">
    ...
    <p>Upgrade to unlock all premium engineering calculators</p>
</div>
...
<div class="plans">
    <div class="plan featured">
        ...
        <div class="plan-detail">For independent engineers and consultants</div>
        <div class="plan-feat">Full access to all Pro calculators</div>
        <div class="plan-feat">Save and load calculation sessions</div>
        <div class="plan-feat">Export results to CSV and PDF</div>
        <div class="plan-feat">Priority feature requests</div>
    ...
    <div class="plan">
        ...
        <div class="plan-feat">Full Pro access for your team</div>
        <div class="plan-feat">Department Head (HOD) management</div>
        <div class="plan-feat">Role-based employee access</div>
        <div class="plan-feat">Organization-wide usage dashboard</div>
        <div class="plan-feat">Dedicated technical support</div>
        <button class="btn-sub btn-org" onclick="subscribe('organization')">Contact Sales</button>
    </div>
</div>
<div style="text-align:center">
    <a class="back" href="/">← Back to Calculator List</a>
...
async def login(request: Request):
    if not GOOGLE_CLIENT_ID:
        return HTMLResponse(
            ...
            "Google Sign-In is not configured for this deployment.</h2>"
        )
...
<div class="hdr">
    <a href="/" style="color:#94a3b8;text-decoration:none;font-size:13px;padding:6px 14px;border-radius:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);font-weight:500">← Home</a>
...
<div class="card">
<div class="card-t">Dashboard Access &amp; Pricing</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
    <div><label style="font-size:11px;color:#9aa2c0;margin-bottom:4px;display:block">Individual Pro Plan (per month)</label>
...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px">
    <div><label style="font-size:10px;color:#6b7394;margin-bottom:4px;display:block">Base Price (per month)</label>
        <input class="inp" type="number" id="org_base" value="{pricing.get('org_base',9999)}" step="1"></div>
    <div><label style="font-size:10px;color:#6b7394;margin-bottom:4px;display:block">Included Users</label>
        <input class="inp" type="number" id="org_base_u" value="{pricing.get('org_base_users',50)}" step="1"></div>
    <div><label style="font-size:10px;color:#6b7394;margin-bottom:4px;display:block">Additional Block Price</label>
        <input class="inp" type="number" id="org_extra" value="{pricing.get('org_extra',5000)}" step="1"></div>
    <div><label style="font-size:10px;color:#6b7394;margin-bottom:4px;display:block">Users Per Block</label>
...
<div class="card">
<div class="card-t">Complimentary Access — Free Lifetime Users ({len(free_emails)}/20)</div>
<p style="font-size:12px;color:#6b7394;margin-bottom:12px">Listed users receive complimentary Pro access without payment. Maximum of 20 entries.</p>
{free_rows}
...
<div class="card">
<div class="card-t">Organization Subscriptions</div>
<p style="font-size:12px;color:#6b7394;margin-bottom:12px">Manage organization subscriptions. The organization administrator can add Department Heads (HODs), who in turn can add team members.</p>
{org_rows}
...
<div class="card">
<div class="card-t">Platform Administrators</div>
...
async def user_dashboard(request: Request):
    ...
    html = f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>My Account — CementCalc</title>
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
...
<div class="hdr">
    <a href="/" style="color:#94a3b8;text-decoration:none;font-size:13px;padding:6px 14px;border-radius:8px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);font-weight:500">← Home</a>
    <div><h1>My Account</h1><p>Profile, Access &amp; Subscription</p></div>
...
<div class="card">
<div class="card-t">Saved Calculation Sessions</div>
<p style="font-size:12px;color:#6b7394;margin-bottom:12px">Calculation inputs and scenarios are saved locally in your browser. Use the Save and Load buttons on each dashboard to manage them.</p>
<button onclick="clearAllSessions()" style="padding:8px 16px;border-radius:8px;font-size:12px;font-weight:600;border:1px solid rgba(239,68,68,0.3);background:rgba(239,68,68,0.1);color:#ef4444;cursor:pointer;font-family:inherit">Clear All Saved Sessions</button>
</div>
</div>
<script>
function clearAllSessions(){{if(!confirm('Are you sure you want to clear all saved calculation sessions?'))return;const keys=Object.keys(localStorage).filter(k=>k.startsWith('cc_'));keys.forEach(k=>localStorage.removeItem(k));alert('Cleared '+keys.length+' saved sessions.')}}
</script>
</body></html>"""
    return HTMLResponse(content=html)
