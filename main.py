...
async def serve_dashboard(request: Request, dashboard_id: str):
    ...
    <div id="cc-nav" style="
        ...
        border-bottom:2px solid {dash['color']};
    ...
    ">
        ...
        <span style="color:{dash['color']}; font-size:14px; font-weight:600;">{dash['icon']} {dash['title']}</span>
        {"<span style='margin-left:auto;font-size:11px;color:#64748b;'>Logged in as " + user['name'] + "</span>" if user else ""}
    </div>
    <div style="height:46px;"></div>
    """

    html_content = html_content.replace("<body>", f"<body>{nav_bar}", 1)
    if "<body " in html_content and nav_bar not in html_content:
        import re
        html_content = re.sub(r"(<body[^>]*>)", rf"\1{nav_bar}", html_content, count=1)
