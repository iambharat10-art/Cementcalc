...
{"<p style='font-size:13px;color:#a78bfa'>Current access: <strong style=color:#22c55e>PRO ✓</strong> " + ("(Admin)" if user_is_admin else "(Free Lifetime)" if user.get("email","").lower() in [e.lower() for e in config.get("free_emails",[])] else "(Organization)") + "</p>" if has_pro_access(request) else "<p style='font-size:13px;color:#9aa2c0'>Current plan: <strong style=color:#6b7394>Free</strong></p><p style='font-size:12px;color:#6b7394;margin-top:8px'>Upgrade to Pro for full access to all engineering dashboards.</p><a href='/pricing' style='display:inline-block;margin-top:10px;padding:8px 20px;background:#8b5cf6;color:#fff;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600'>Upgrade to Pro</a>"}
</div>
